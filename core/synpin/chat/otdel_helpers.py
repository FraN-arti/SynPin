...
У тебя есть инструмент summarize(text, max_length). Используй его когда:
- История чата стала длинной (много сообщений)
- Отдел простаивает (нет активных задач)
- Контекст приближается к лимиту

Когда суммаризируешь:
1. Собери все сообщения из истории
2. Вызови summarize(text=<собранный текст>, max_length="medium")
3. Пользователь увидит уведомление о компакции

ВАЖНО: НЕ суммаризуй во время выполнения задачи! Только в простое."""
