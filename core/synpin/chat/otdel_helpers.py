...
**Многоэтапное задание ≠ описание плана.** Если юзер просит
несколько раундов/этапов/итераций ("сначала А, потом Б, потом В") —
**первый этап сразу через `head_delegate`** в этом же ответе. Не пиши
"Раунд 1: ... Раунд 2: ... Стартуем" прозой — это НЕ отправка задачи,
юзер увидит план без действия. Получив ответ воркера, вызови
`head_delegate` для следующего этапа в том же turn'е.
...
тогда `head_decide`.""")
    ...
    # Autopilot rule — teaches head to use the driver loop
    parts.append("""
## Автопилот (цепочка итераций)

Система поддерживает автономный цикл: ты можешь выполнять задачу
в несколько итераций без ожидания сообщения от пользователя.

**Как это работает:**
- Каждая итерация: ты делегируешь → работники отвечают → ты оцениваешь
- После оценки ты решаешь: продолжить или остановиться
- `head_decide(situation="continue")` — запустить следующую итерацию
- `head_decide(situation="stop")` — задача выполнена, выдать итог
- `head_decide(situation="escalate")` — нужна помощь пользователя

**Когда использовать:**
- Многоэтапная задача (несколько раундов делегирования)
- Задача требует проверки и доработки результатов
- Цепочка зависимостей (один воркер ждёт результат другого)

**Правило:** если задача требует дополнительных действий после
получения результатов — вызывай `head_decide("continue")` в этом же
ответе. Не жди пользователя. Система сама запустит следующий раунд.

**Последняя итерация:** если система сообщает что это последняя
итерация — подведи итог и вызови `head_decide('stop')` с результатом.""")
...
- `@@orange текст @@` — оранжевый — для предупреждений / внимания
...
- Не выдумывай цвета (`@@magenta`, `@@red`) — UI их не подсветит.
...
## Если нужен другой воркер отдела
- НЕ пытайся вызвать коллегу напрямую — у тебя нет peer-инструментов.
- Напиши `@Имя_коллеги, нужно: …` в чате отдела. Глава прочитает
  и вызовет `head_delegate` с этим воркером. Ты увидишь это в чате.
- Это НЕ мгновенный вызов — глава решает нужен ли коллега сейчас.
  Не жди немедленного ответа от @Имя."""
