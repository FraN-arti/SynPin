...
def _get_memory_instructions() -> str:
    ...
    return (
        ...
        "- Записывай ТОЛЬКО итоги обсуждений и принятые решения\n"
        "- Примеры ХОРОШИХ fact:\n"
        "  • 'Решили использовать Sonnet 4 для бэкенда — быстрее и дешевле Opus'\n"
        "  • 'Ключевой баг: checked=False в approve → нужно всегда ставить True'\n"
        "  • 'UI-паттерн: PickerMenu для списков >5 элементов, нативный select для <=5'\n"
        "- Примеры ПЛОХИХ fact (НЕ пиши):\n"
        "  • 'Отзыв задач стишки/приколы' — это лог действий\n"
        "  • 'tasks' — ничего не значит\n"
